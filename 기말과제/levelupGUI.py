...
def memory_game(level):
    ...
    window = sg.Window("기억력 테스트", layout_show, element_justification='center', finalize=True, keep_on_top=True, margins=(40, 20))
    window.refresh()
    time.sleep(3)
    window.close()
    ...
    window = sg.Window("답 입력", layout_input, element_justification='center', finalize=True, keep_on_top=True, margins=(30, 20))
    while True:
...
def main():
    ...
    while True:
        ...
        if success:
            ...
            show_popup("💀 게임 오버", [
                "틀렸습니다!",
                f"정답은: {' '.join(map(str, correct))}",
                f"도달한 최고 레벨: {level}"
            ], button_text="종료", highlight=[0])  
            break
